...
// Cached functions for structural matching
...
export function tryStructuralMatch(
  ...
  const normalizedCurrent = cachedNormalizeValue(anchorValue);
  ...
  for (const labelEntry of labels) {
    ...
    if (
      ...
      const normalizedCurrentStr = cachedToComparableString(normalizedCurrent);

      // Check if the primitive matches any property value in the registered object
      const matchingProperty = Object.entries(labelObject).find(
        ([_key, propValue]) => {
          return cachedToComparableString(cachedNormalizeValue(propValue)) === normalizedCurrentStr;
        }
      );
