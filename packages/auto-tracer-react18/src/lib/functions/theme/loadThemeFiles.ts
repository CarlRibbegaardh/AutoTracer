/**
 * Loads and merges React18 theme files from a directory following the 3-step hierarchy.
 *
 ...
 * - Step 1: `*react-theme.json` (base theme, applies to both light and dark modes)
 * - Step 2: `*react-theme-light.json` (light mode override)
 * - Step 3: `*react-theme-dark.json` (dark mode override)
 *
 * Supports named themes (e.g., `colorblind-react-theme.json`) without requiring renaming.
 ...
 * @returns Merged React theme configuration from all found theme files, or empty object if none found
 *
 * @example
 * ```typescript
 ...
 * const finalColors = mergeThemes({
 *   ...themeFromFiles,
 *   ...programmaticConfig?.colors
 * });
 * ```
 *
 * @remarks
...
export async function loadThemeFiles(
  searchDir: string
): Promise<Partial<NonNullable<ReactTracerOptions["colors"]>>> {
  ...
  const loadJsonFile = (
    filename: string
  ): Partial<NonNullable<ReactTracerOptions["colors"]>> => {
    try {
      ...
      if (errors.length > 0) {
        internalLogger.warn(
          `[ReactTracer] Theme validation errors in "${filename}":`
        );
        errors.forEach((themeError) => {
          internalLogger.warn(`  - ${themeError}`);
        });
      }
      ...
      internalLogger.warn(
        `[ReactTracer] Warning: Failed to parse theme file "${filename}". Using defaults.`,
  ...
  // Step 1: Load base theme (*react-theme.json)
  const baseFile = findFirstMatch(/^[^.]*react-theme\.json$/);
  let result: Partial<NonNullable<ReactTracerOptions["colors"]>> = baseFile
    ? loadJsonFile(baseFile)
    : {};

  // Step 2: Merge light mode override (*react-theme-light.json)
  const lightFile = findFirstMatch(/^[^.]*react-theme-light\.json$/);
  if (lightFile) {
  ...
  // Step 3: Merge dark mode override (*react-theme-dark.json)
  const darkFile = findFirstMatch(/^[^.]*react-theme-dark\.json$/);
  if (darkFile) {
