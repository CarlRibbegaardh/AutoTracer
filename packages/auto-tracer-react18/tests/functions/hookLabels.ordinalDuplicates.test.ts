...
import { addLabelForGuid, clearAllHookLabels, getLabelsForGuid, resolveHookLabel } from "../../src/lib/functions/hookLabels";
...
describe("hookLabels - ordinal resolution for identical object structures", () => {
  ...
  it("should resolve two hooks with identical structure using ordinal position", () => {
    ...
    const resolvedCustom = resolveHookLabel(2, customHookValue, { guid, allAnchors: allFiberAnchors });
    expect(resolvedCustom).toBe("customHookResult");

    const resolvedNested = resolveHookLabel(3, nestedHookValue, { guid, allAnchors: allFiberAnchors });
  ...
  it("should handle identical object values with different string properties", () => {
    ...
    const resolved1 = resolveHookLabel(0, hook1, { guid, allAnchors: allFiberAnchors });
    const resolved2 = resolveHookLabel(1, hook2, { guid, allAnchors: allFiberAnchors });
