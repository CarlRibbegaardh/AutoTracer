...
describe("hookLabels", () => {
  ...
  describe("resolveHookLabel - Scenario 1: Unique Values", () => {
    it("should match label by unique value", () => {
      ...
      // Test resolution
      expect(resolveHookLabel(0, emptyArray, { guid, allAnchors })).toBe(
        "filteredTodos"
      );
      expect(resolveHookLabel(1, false, { guid, allAnchors })).toBe("loading");
      expect(resolveHookLabel(2, 42, { guid, allAnchors })).toBe("unknown");
    });
  });
  ...
  describe("resolveHookLabel - Scenario 2: Duplicate Values, All Labeled", () => {
    it("should match by ordinal position when all occurrences are labeled", () => {
      ...
      expect(resolveHookLabel(1, emptyArray, { guid, allAnchors })).toBe(
        "completedTodos"
      );
      expect(resolveHookLabel(2, emptyArray, { guid, allAnchors })).toBe(
        "archivedTodos"
      );
    });
  });
  ...
  describe("resolveHookLabel - Multiple Unlabeled Hooks", () => {
    it("should return plain 'unknown' for each unlabeled hook (not a union)", () => {
      ...
      expect(resolveHookLabel(0, emptyArray, { guid, allAnchors })).toBe("unknown");
      expect(resolveHookLabel(1, emptyArray, { guid, allAnchors })).toBe("unknown");
      expect(resolveHookLabel(2, emptyArray, { guid, allAnchors })).toBe("unknown");
    });
  });
  ...
  describe("resolveHookLabel - Object-returning hooks with functions", () => {
    it("should resolve unique object via normalized direct compare (functions treated as (fn))", () => {
      ...
      expect(resolveHookLabel(2, allAnchors[2]!.value, { guid, allAnchors })).toBe(
        "custom"
      );
    });
    ...
    it("should perform ordinal mapping for duplicate objects using normalized compare", () => {
      ...
      expect(resolveHookLabel(0, a0, { guid, allAnchors })).toBe("firstCustom");
      expect(resolveHookLabel(1, a1, { guid, allAnchors })).toBe("secondCustom");
    });
    ...
    it("should resolve object label when current anchor value is primitive via reconstruction", () => {
      ...
      expect(resolveHookLabel(2, "pattern-custom", { guid, allAnchors })).toBe("custom");
    });
  });
});
