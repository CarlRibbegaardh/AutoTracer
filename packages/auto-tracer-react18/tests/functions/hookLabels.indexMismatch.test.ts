...
describe("hookLabels - index mismatch with custom hooks containing internal state", () => {
  ...
  it("should fail to resolve when label indexes don't match fiber indexes", () => {
    ...
    const resolvedDesc = resolveHookLabel(1, "pattern-test", { guid, allAnchors: allFiberAnchors });
    const resolvedCounter = resolveHookLabel(2, 10, { guid, allAnchors: allFiberAnchors });
    const resolvedCustom = resolveHookLabel(3, customHookValue, { guid, allAnchors: allFiberAnchors });
    const resolvedNested = resolveHookLabel(4, nestedHookValue, { guid, allAnchors: allFiberAnchors });
