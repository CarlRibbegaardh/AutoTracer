...
describe("fiberTraversal", () => {
  ...
  describe("walkFiberTree", () => {
    ...
    it("should warn and return when max depth exceeded", async () => {
      ...
      mockTraceOptions.maxFiberDepth = 2;
      ...
      expect(logWarn).toHaveBeenCalledWith(
        "ReactTracer: Maximum traversal depth (2) reached, stopping to prevent stack overflow",
      );
      expect(visitor.visit).not.toHaveBeenCalled();
    });

    it("should fall back to default max depth (500) when traceOptions value is undefined", async () => {
      const { logWarn } = await import("@src/lib/functions/log.js");

      mockTraceOptions.maxFiberDepth = undefined;
      ...
      walkFiberTree(fiber, 501, visitor);

      expect(logWarn).toHaveBeenCalledWith(
        "ReactTracer: Maximum traversal depth (500) reached, stopping to prevent stack overflow",
      );
      expect(visitor.visit).not.toHaveBeenCalled();
    });
  ...
  describe("shouldSkipFiber", () => {
    it("should return false when includeNonTrackedBranches is enabled", async () => {
      mockTraceOptions.includeNonTrackedBranches = true;
    ...
    it("should return false for tracked fibers", async () => {
      mockTraceOptions.includeNonTrackedBranches = false;
