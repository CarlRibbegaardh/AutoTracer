...
import {
  ...
  logDevToolsStatus,
  restoreRenderHook,
} from "@src/lib/functions/devToolsUtils";
...
describe("devToolsUtils", () => {
  ...
  describe("getDevToolsHook", () => {
    ...
    it("should handle errors gracefully and return null", async () => {
      const { internalLogger } = vi.mocked(await import("@logger/internalLogger.js"));
  ...
  describe("installRenderHook", () => {
    ...
    it("should handle errors and return null", async () => {
      ...
      expect(internalLogger.error).toHaveBeenCalledWith(
        "ReactTracer: Error installing render hook:",
        expect.any(Error)
      );
    });
  });
  ...
  describe("restoreRenderHook", () => {
    ...
    it("should handle errors and return false", async () => {
      ...
      expect(result).toBe(false);
      expect(internalLogger.error).toHaveBeenCalledWith(
        "ReactTracer: Error restoring render hook:",
        expect.any(Error)
      );
    });
  });
  ...
  describe("createSafeRenderHook", () => {
    ...
    it("should log errors when hook function throws", async () => {
      const { internalLogger } = vi.mocked(await import("@logger/internalLogger.js"));
      ...
      expect(internalLogger.error).toHaveBeenCalledWith(
        "ReactTracer: Error in render hook:",
        error
      );
    });

    it("should not log errors when safeHook silently catches", async () => {
      const mockHookFn = vi.fn(() => {
        throw new Error("Hook error");
      });
      const safeHook = createSafeRenderHook(mockHookFn);
      ...
      // Error is caught and logged, test just verifies no throw
    });
  });

  describe("logDevToolsStatus", () => {
    it("should log debug message when DevTools is available", async () => {
      const { internalLogger } = vi.mocked(await import("@logger/internalLogger.js"));
      ...
      expect(internalLogger.debug).toHaveBeenCalledWith("ReactTracer: React DevTools detected");
    });

    it("should log warning when DevTools is not available", async () => {
      const { internalLogger } = vi.mocked(await import("@logger/internalLogger.js"));
      ...
      expect(internalLogger.warn).toHaveBeenCalledWith(
        "ReactTracer: React DevTools not available. To use ReactTracer, either:\r\n" +
          ...
          "  2. Use the @autotracer/plugin-vite-react18 or @autotracer/plugin-babel-react18 plugin\r\n" +
