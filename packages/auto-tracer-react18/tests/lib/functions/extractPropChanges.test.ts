...
import { extractPropChanges } from "@src/lib/functions/extractPropChanges";
...
describe("extractPropChanges", () => {
  ...
  it("should detect simple prop changes", async () => {
    const { isReactInternal } = vi.mocked(
      await import("@src/lib/functions/isReactInternal.js")
    );
    const { getSkippedProps } = vi.mocked(
      await import("@src/lib/functions/getSkippedProps.js")
    );
