...
describe("deepMergeOptions", () => {
  ...
  describe("shallow properties", () => {
    ...
    it("should merge filterEmptyNodes", () => {
      const result = deepMergeOptions(defaultOptions, {
        filterEmptyNodes: "none",
      });

      expect(result.filterEmptyNodes).toBe("none");
    });

    it("should merge includeRendered", () => {
      const result = deepMergeOptions(defaultOptions, {
        includeRendered: "always",
      });

      expect(result.includeRendered).toBe("always");
    });
  ...
  describe("nested colors object", () => {
    ...
    it("should handle missing nested color properties", () => {
      const optionsWithoutColors: ReactTracerOptions = {
        ...defaultOptions,
        colors: undefined,
      };
    ...
    it("should preserve all nested color configurations when merging", () => {
      ...
      const optionsWithCustomColors: ReactTracerOptions = {
        ...defaultOptions,
        colors: customColors,
      };
