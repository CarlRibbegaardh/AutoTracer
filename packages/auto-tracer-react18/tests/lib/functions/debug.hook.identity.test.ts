...
import { extractUseStateValues } from "../../../src/lib/functions/extractUseStateValues";
import { findStatefulHookAnchors } from "../../../src/lib/functions/hookMapping/findStatefulHookAnchors";
import type { Hook } from "../../../src/lib/functions/hookMapping/types";
import { clearAllHookLabels } from "../../../src/lib/functions/hookLabels";
...
describe("Hook Identity Debug", () => {
  ...
  function createFiberWithUseState(hooks: Hook[], alternate?: FiberNode | null) {
    const memoizedState = chainHooks(hooks);

    return {
      memoizedState,
      alternate: alternate ?? null,
      _debugHookTypes: hooks.map(() => {return "useState"}),
    };
  }
  ...
  it("should have matching hook references - same as walkFiberForUpdates does it", () => {
    ...
    meaningfulStateChanges.forEach((change, idx) => {
      ...
      // Manual check
      const manualCheck = anchors.findIndex((a) => {return a === change.hook});
