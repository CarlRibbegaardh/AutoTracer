...
describe("treeProcessing rendering - custom hook labels resolve for object-returning custom hooks", () => {
   ...
   * const logger = useReactTracer({ name: "LabelHooksTestComponent" });
  ...
  function createFiber(
    ...
    const hooks = hookValues.map((v) => {return createUseStateHook(v)});
    ...
    return {
      ...
      pendingProps: {},
      memoizedState: chain,
      _debugHookTypes: ["useRef", ...hooks.map(() => {return "useState"})],
    };
  }
  ...
  it("should label primitive hooks AND object-returning custom hooks on mount", () => {
    ...
    const hasTitle = output.some(l => {return l.includes("Initial state title:")});
    const hasCount = output.some(l => {return l.includes("Initial state count:")});
    expect(hasTitle).toBe(true);
    expect(hasCount).toBe(true);

    // Expect custom/nested now resolved (no 'unknown')
    const hasCustomResolved = output.some(l => {return l.includes("Initial state custom:") && l.includes("test-custom")});
    const hasNestedResolved = output.some(l => {return l.includes("Initial state nested:") && l.includes("nested-custom")});
    expect(hasCustomResolved).toBe(true);
    expect(hasNestedResolved).toBe(true);

    // Ensure we did not log them as unknown anymore
    const anyUnknownWithCustomValues = output.some(l => {return l.includes("Initial state unknown:") && (l.includes("test-custom") || l.includes("nested-custom"))});
