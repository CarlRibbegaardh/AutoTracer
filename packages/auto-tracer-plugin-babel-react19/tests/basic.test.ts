...
const inputCode = `
  // no useReactTracer import here initially
...
describe("@autotracer/plugin-babel-react19", () => {
  it("injects useReactTracer into a component with hooks (opt-out mode)", () => {
    ...
    try {
      ...
      // Expect import to be present or a call to useReactTracer injected
      expect(code).toMatch(/useReactTracer\(/);
    } finally {
      process.env.NODE_ENV = originalNodeEnv;
    }
  });

  it("transforms in production mode", () => {
    ...
    try {
      ...
      const code = result?.code ?? "";
      // Should contain auto-tracer imports or calls
      expect(code).toMatch(/useReactTracer\(/);
    } finally {
      process.env.NODE_ENV = originalNodeEnv;
    }
  ...
  it("respects TRACE_INJECT=0 environment variable", () => {
    ...
    try {
      ...
      expect(code).not.toMatch(/import.*useReactTracer/);
      expect(code).not.toMatch(/const.*useReactTracer/);
  ...
  it("forwards prefix option to core as component name prefix", () => {
    const originalNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "development";
    ...
    try {
      const result = transformSync(inputCode, {
        filename: "Hello.tsx",
        plugins: [[plugin, { mode: "opt-out", prefix: "Island1" }]],
      ...
      const code = result?.code ?? "";
      expect(code).toContain("Island1:Hello");
    } finally {
      process.env.NODE_ENV = originalNodeEnv;
    }
  });
