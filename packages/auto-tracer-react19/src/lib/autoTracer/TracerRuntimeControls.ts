...
export interface TracerRuntimeControls {
  ...
  /**
   * Set auto-stop limit.
   * When set, tracer will automatically stop after reaching the limit.
   *
   * Side effects:
   * - Persists to localStorage when available.
   *
  ...
  /**
   * Get current count (renders for React, calls for Flow).
   *
   * @returns Current count since start
   */
   ...
   * Tracing begins when a component matching this pattern renders.
   *
   * Side effects:
   * - Persists to localStorage when available.
   *
   ...
   * Tracing stops when a component matching this pattern renders (see endTriggerMode).
   *
   * Side effects:
   * - Persists to localStorage when available.
   *
   ...
   * - Persists to localStorage when available.
   *
   * @param mode - "on-entry" fires when component renders, "on-exit" fires after render completes
   */
