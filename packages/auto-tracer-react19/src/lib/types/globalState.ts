...
/**
 * Gets the shared legacy tracer-installed flag.
 *
 * Originally, installing the global render hook and activating tracing were the
 * same operation, so this flag represented both states. Passive mode later
 * separated those states: the render hook can now remain installed while this
 * flag is false. The flag therefore currently represents active tracing rather
 * than hook installation.
 *
 * This state should be refactored to use a name that describes active tracing.
 *
 * @returns True while React tracing is active; otherwise false
 */
