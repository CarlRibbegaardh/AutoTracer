...
export const createConsoleGroupRenderer = (): TreeRenderer => {
  return (nodes: readonly TreeNode[]) => {
    ...
    const enableInternals = getTraceOptions().showLevelDetails ?? false;
    const showFlags = getTraceOptions().showFlags ?? false;

    const runtimeFiltering = getReactRuntimeFilteringState();
    const isFilterModeEnabled = runtimeFiltering.isFilterModeEnabled();
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.renderType === "Marker" && enableInternals) {
        ...
        log(`... (Level: ${nextNodeDepth}, Filtered nodes: ${filteredCount})`);
        continue;
      }
      ...
      const action =
        isFilterModeEnabled && node.renderType !== "Marker"
          ? runtimeFiltering.createFilterAction(node.displayName)
          : undefined;

      logAction.logFn(...logAction.args);

      const filterSnippet = getStringOrUndefined(action);
      if (filterSnippet) {
        log(`%c${filterSnippet}`, "font-size: 0.85em; font-weight: 400;");
      }
