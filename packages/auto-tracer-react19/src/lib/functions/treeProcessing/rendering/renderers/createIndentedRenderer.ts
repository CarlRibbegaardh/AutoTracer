...
export const createIndentedRenderer = (): TreeRenderer => {
  return (nodes) => {
    internalLogger.debug(
      `createIndentedRenderer: Processing ${nodes.length} nodes`
    );
    ...
    for (let i = 0; i < nodes.length; i++) {
      try {
        ...
        internalLogger.error(`Error rendering node ${i}:`, error);

        // Continue rendering other nodes
      }
    }

    internalLogger.debug("createIndentedRenderer: Complete");
  };
};
