...
export function renderPropChange(
  ...
  if (isMount) {
    ...
    if (isObjectMode) {
      const value = prepareValue(change.value, isObjectMode);
      return {
        level: "prop-initial",
    ...
    // Update: show change
    if (isObjectMode) {
      const prev = prepareValue(change.prevValue, isObjectMode);
      const curr = prepareValue(change.value, isObjectMode);
      const simple =
