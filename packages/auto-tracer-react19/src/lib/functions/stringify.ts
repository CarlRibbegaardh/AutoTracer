 ...
 *
 * Configurable stringify wrapper for reactTracer with function instance tracking
...
import { normalizeValueDeepWithIDs } from "./normalization/normalizeValueDeepWithIDs.js";
import { withCache } from "./functionCache/withCache.js";
import { internalLogger } from "@logger/internalLogger.js";

// Cached normalizeValueDeepWithIDs for stringify
const cachedNormalizeValueDeepWithIDs = withCache(
...
/**
 * Main stringify function used by reactTracer for display and debugging.
 *
...
export function stringify(value: unknown): string {
  const h = internalLogger.enter("stringify");
  try {
    ...
    const normalized = cachedNormalizeValueDeepWithIDs(value);
