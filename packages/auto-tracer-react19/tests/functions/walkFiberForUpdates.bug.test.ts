...
describe('walkFiberForUpdates - label matching bug', () => {
  ...
  it.skip('should correctly match labels when a hook without queue exists between labeled hooks', () => {
    ...
    // 0: useReactTracer() - injected, HAS queue
    ...
    console.log('Extracted hooks with queues:', extractedHooks.map(h => {return h.name}));
    ...
    expect(extractedHooks[0]?.name).toBe('state0'); // reactTracer
  ...
  addLabelForGuid(guid, { label: 'loading', index: 18, value: false });
    ...
    // Get the second extracted hook (first one after reactTracer)
