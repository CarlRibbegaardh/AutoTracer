...
describe("treeProcessing rendering - ordinal resolution for duplicate object structures", () => {
   ...
   * const logger = useReactTracer({ name: "LabelHooksPatternTestComponent" });
  ...
  it("should distinguish two custom hooks with identical object structure using ordinal position", () => {
    ...
    const hasDescription = output.some(l => {return l.includes("Initial state description:") && l.includes("pattern-test")});
    const hasCounter = output.some(l => {return l.includes("Initial state counter:") && l.includes("10")});
    ...
    const hasCustomHookResult = output.some(l =>
      {return l.includes("Initial state customHookResult:") && l.includes("pattern-custom")}
    );
    ...
    const hasNestedHookResult = output.some(l =>
      {return l.includes("Initial state nestedHookResult:") && l.includes("nested-custom")}
    );
    ...
    const nestedMislabeled = output.some(l =>
      {return l.includes("Initial state customHookResult:") && l.includes("nested-custom")}
    );
