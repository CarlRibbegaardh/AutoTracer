...
import { matchesPattern } from "../../src/functions/config/matchesPattern";
import { shouldProcessFile } from "../../src/functions/config/shouldProcessFile";
import { isComponentFunction } from "../../src/functions/detect/isComponentFunction";
import { extractComponentInfo } from "../../src/functions/detect/extractComponentInfo";
import { hasExistingUseReactTracerImport } from "../../src/functions/detect/hasExistingUseReactTracerImport";
...
function makeComponentWithReactTracer(): string {
  return `
    function MyComponent() {
...
describe("labelHooks (list-based)", () => {
  it("labels useState/useReducer and configured selectors, but not custom hooks", () => {
    ...
    expect(out).toMatch(/__reactTracer\.labelState\(0,\s*['"]title['"]/);
    expect(out).toMatch(/__reactTracer\.labelState\(1,\s*['"]count['"]/);

    // Should label configured selectors with alternating name-value pairs
    expect(out).toMatch(/__reactTracer\.labelState\(2,\s*['"]todos['"]/);
    expect(out).toMatch(/__reactTracer\.labelState\(3,\s*['"]appTodos['"]/);

    // Should NOT label custom hooks when only list is provided
    expect(out).not.toMatch(/__reactTracer\.labelState\(['"]custom['"]/);
    expect(out).not.toMatch(/__reactTracer\.labelState\(['"]nested['"]/);
...
describe("labelHooksPattern (regex-based)", () => {
  it("labels built-ins plus all hooks matching the pattern", () => {
    ...
    expect(out).toMatch(/__reactTracer\.labelState\(2,\s*['"]todos['"]/);
    expect(out).toMatch(/__reactTracer\.labelState\(3,\s*['"]appTodos['"]/);
    expect(out).toMatch(/__reactTracer\.labelState\(4,\s*['"]custom['"]/);
    expect(out).toMatch(/__reactTracer\.labelState\(5,\s*['"]nested['"]/);
  ...
  it("should fail gracefully when pattern includes regex delimiters (common mistake)", () => {
    // User mistakenly includes / delimiters like they would in JavaScript literal regex
    const code = `
      function MyComponent() {
        const [s] = useState(0);
        const a = useAlpha();
    ...
    const context: TransformContext = {
      ...
      config: {
        ...
        exclude: { paths: ["**/*.test.*", "**/*.spec.*"] },
        labelHooks: [],
        // COMMON MISTAKE: includes regex delimiters - should be auto-sanitized
        labelHooksPattern: "/^use[A-Z].*/",
        serverComponents: false,
      },
    };
    ...
    // FIXED: Delimiters are now automatically stripped, pattern works correctly
    const out = result.code;

    // All hooks should be labeled
    expect(out).toMatch(/labelState\(0,\s*['"]s['"]/);
    expect(out).toMatch(/labelState\(1,\s*['"]a['"]/);
    expect(out).toMatch(/labelState\(2,\s*['"]b['"]/);

    // 3 labelState calls (useState + useAlpha + useBeta)
    expect(countOccurrences(out, "labelState")).toBe(3);
  });
});

describe("useReactTracer exclusion", () => {
  it("should not label useReactTracer hook", () => {
    const code = makeComponentWithReactTracer();
    const context: TransformContext = {
      filename: "src/MyComponent.tsx",
      config: {
        mode: "opt-out",
    ...
    // Should NOT label useReactTracer
...
describe("pragma controls", () => {
  it("skips transformation when @trace-disable pragma is present", () => {
    ...
    // Note: Babel may normalize whitespace even when skipping transformation
  });
...
describe("HOC-wrapped components", () => {
  it("handles memo-wrapped components", () => {
    ...
    expect(result.code).toMatch(/__reactTracer\.labelState\(0,\s*['"]count['"]/);
  });
  ...
  it("handles forwardRef-wrapped components", () => {
    ...
    expect(result.components).toHaveLength(1);
    expect(result.components[0].name).toBe("MyComponent");
    expect(result.code).toMatch(/__reactTracer\.labelState\(0,\s*['"]value['"]/);
  });
  ...
  it("handles nested HOC wrapping", () => {
    ...
    expect(result.components).toHaveLength(1);
    expect(result.components[0].name).toBe("MyComponent");
    expect(result.code).toMatch(/__reactTracer\.labelState\(0,\s*['"]data['"]/);
  });
...
describe("non-component files", () => {
  it("does not transform non-component functions (no JSX)", () => {
    ...
    expect(result.code).not.toMatch(/useReactTracer/);
    expect(result.code).not.toMatch(/labelState/);
  });
});

describe("edge cases in injectIntoBlockStatement", () => {
  it("handles bare useReactTracer calls without assignment", () => {
    const code = `
      function MyComponent() {
        useReactTracer({ name: "Test" });
        const [count, setCount] = useState(0);
        return <div>{count}</div>;
      }
    `;
    ...
    const result = transform(code, context);
    expect(result.injected).toBe(true);
    // Should not inject labels when there's a bare useReactTracer call
    expect(result.code).not.toMatch(/labelState/);
  });
  ...
  it("still injects labels when both a tracer variable and a separate bare call exist", () => {
    const code = `
      function MyComponent() {
        const tracer = useReactTracer({ name: "A" });
        useReactTracer({ name: "B" });
        const [count] = useState(0);
        return <div>{count}</div>;
      }
    `;
  ...
  it("skips labeling when labelState calls already exist", () => {
    const code = `
      function MyComponent() {
        const tracer = useReactTracer({ name: "Test" });
  ...
  it("handles components with no hooks to label", () => {
    ...
    const result = transform(code, context);
    expect(result.injected).toBe(true);
    // Should inject useReactTracer but no labelState calls
    expect(result.code).toMatch(/useReactTracer/);
    expect(result.code).not.toMatch(/labelState/);
  });
  ...
  it("does not inject duplicate tracer when tracer variable already exists and no labels to add", () => {
    ...
    expect(countOccurrences(result.code, "const tracer = useReactTracer")).toBe(
      1
    );
    // Import may be injected once since transform marks as injected
    expect(countOccurrences(result.code, "import { useReactTracer }")).toBe(1);
    expect(result.code).not.toMatch(/labelState/);
  });
  ...
  it("uses existing tracer identifier for labelState when present", () => {
    const code = `
      function MyComponent() {
        const tracer = useReactTracer({ name: "Comp" });
        const [count] = useState(0);
        return <div>{count}</div>;
      }
    `;
    ...
    const result = transform(code, context);
    expect(result.injected).toBe(true);
    // Should label using the 'tracer' identifier, not __reactTracer
    expect(result.code).toMatch(/tracer\.labelState\(0,\s*['"]count['"]/);
    expect(result.code).not.toMatch(/__reactTracer\.labelState/);
  });
...
describe("import handling", () => {
  it("adds useReactTracer import when needed", () => {
    const code = `
      function MyComponent() {
        const [count, setCount] = useState(0);
        return <div>{count}</div>;
  ...
  it("does not add import when useReactTracer is already imported", () => {
    const code = `
      import { useReactTracer } from '@autotracer/react18';
  ...
  it("uses configured importSource when injecting", () => {
    ...
    expect(result.injected).toBe(true);
    expect(result.code).toMatch(
      /import \{ useReactTracer \} from ['"]my-tracer-src['"]/
    );
  });
...
describe("config utilities", () => {
  describe("normalizeConfig", () => {
    it("returns default config when no config provided", () => {
      ...
      expect(config.importSource).toBe("@autotracer/react18");
      expect(config.labelHooks).toEqual([]);
      expect(config.labelHooksPattern).toEqual("^use[A-Z].*");
    });
    ...
    it("merges provided config with defaults", () => {
      ...
      expect(config.importSource).toBe("@autotracer/react18"); // default
    });
  });
  ...
  describe("shouldProcessFile", () => {
    ...
    it("does not process when include excludes the file extension", () => {
      const config = normalizeConfig({ include: { paths: ["**/*.jsx"] }, exclude: { paths: [] } });
...
describe("detect utilities", () => {
  describe("isComponentFunction", () => {
    ...
    it("returns false for non-PascalCase function declarations", () => {
      ...
      // camelCase functions are not components, even with JSX
  ...
  describe("hasExistingUseReactTracerImport", () => {
    it("returns true when useReactTracer is imported", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
            [
            ...
            t.stringLiteral("@autotracer/react18")
          ),
        ])
      );
      expect(hasExistingUseReactTracerImport(ast, "@autotracer/react18")).toBe(
        true
      );
    });

    it("returns false when useReactTracer is not imported", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
            [
    ...
    it("returns false when useReactTracer is imported from a different source", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
            [
        ...
        t.program([
          t.importDeclaration(
            [
              t.importSpecifier(
                t.identifier("alias"),
                t.stringLiteral("useReactTracer") as unknown as t.Identifier
              ),
            ],
            t.stringLiteral("@autotracer/react18")
          ),
    ...
    it("returns false for side-effect import from the source with no specifiers", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration([], t.stringLiteral("@autotracer/react18")),
        ])
      );
      expect(hasExistingUseReactTracerImport(ast, "@autotracer/react18")).toBe(
        false
    ...
    it("returns false for mixed specifiers without named useReactTracer", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
            [
    ...
    it("returns true when mixed specifiers include named useReactTracer", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
            [
    ...
    it("returns false when earlier import from source lacks named useReactTracer even if a later one has it (current behavior)", () => {
      const ast = t.file(
        t.program([
          t.importDeclaration(
  ...
  describe("index.ts exports coverage", () => {
    it("covers all exports from index.ts", () => {
      ...
      expect(typeof hasExistingUseReactTracerImport).toBe("function");
