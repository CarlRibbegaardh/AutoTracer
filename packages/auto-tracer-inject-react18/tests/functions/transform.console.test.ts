...
function countUseReactTracerCalls(code: string): number {
  // Count actual useReactTracer() calls, not imports
  // Match pattern: = useReactTracer( or const __reactTracer = useReactTracer(
  return (code.match(/=\s*useReactTracer\(/g) || []).length;
}
...
describe("Console log injection", () => {
  it("should inject logger.log() after console.log()", () => {
    ...
    expect(out).toMatch(
      /__reactTracer\.log\(['"]Hello world['"],\s*{\s*test:\s*123\s*}\)/
    );

    // Should have exactly one logger.log call
    expect(countOccurrences(out, "__reactTracer.log(")).toBe(1);
  });
  ...
  it("should inject logger.warn() after console.warn()", () => {
    ...
    expect(out).toMatch(/__reactTracer\.warn\(['"]Title is too long['"]/);

    // Should have exactly one logger.warn call
    expect(countOccurrences(out, "__reactTracer.warn(")).toBe(1);
  });
  ...
  it("should inject logger.error() after console.error()", () => {
    ...
    expect(out).toMatch(
      /__reactTracer\.error\(['"]Operation failed['"],\s*error\)/
    );

    // Should have exactly one logger.error call
    expect(countOccurrences(out, "__reactTracer.error(")).toBe(1);
  });
  ...
  it("should inject all three logger methods when all console methods are present", () => {
    ...
    expect(countOccurrences(out, "__reactTracer.labelState(")).toBe(1);
  });
  ...
  it("should inject logger calls even when there are no hooks to label", () => {
    ...
    expect(countOccurrences(out, "__reactTracer.labelState(")).toBe(0);
  });
  ...
  it("should handle multiple console calls of the same type", () => {
    ...
    expect(countOccurrences(out, "__reactTracer.log(")).toBe(3);
  ...
  it("should NOT inject second useReactTracer in nested PascalCase function with console.log", () => {
    const code = `
      function MyComponent() {
        const [count, setCount] = useState(0);
    ...
    // Should have exactly ONE useReactTracer call
    expect(countUseReactTracerCalls(out)).toBe(1);

    // Should NOT have __reactTracer2
    expect(out).not.toContain("__reactTracer2");

    // Should have exactly one __reactTracer variable
    expect(countOccurrences(out, "const __reactTracer")).toBe(1);

    // Should have logger.log call inside HandleClick using parent's __reactTracer
    expect(out).toContain("__reactTracer.log(");
    expect(countOccurrences(out, "__reactTracer.log(")).toBe(1);
  ...
  it("should handle nested arrow function with console.log without duplicate useReactTracer", () => {
    const code = `
      function TodoList() {
    ...
    // Should NOT have __reactTracer2, __reactTracer3, etc.
    expect(out).not.toContain("__reactTracer2");
    expect(out).not.toContain("__reactTracer3");
  ...
  it("should handle nested function returning JSX without treating it as a component", () => {
    ...
    // Should have exactly ONE useReactTracer call (only for ParentComponent)
    expect(countUseReactTracerCalls(out)).toBe(1);

    // RenderHelper should NOT get its own useReactTracer even though it returns JSX
    expect(out).not.toContain("__reactTracer2");

    // Should have logger.log using parent's __reactTracer
    expect(out).toContain("__reactTracer.log(");
    expect(countOccurrences(out, "__reactTracer.log(")).toBe(1);
  });

  it("should NOT inject duplicate useReactTracer inside useCallback with console.log", () => {
    const code = `
      function MyComponent() {
        const [count, setCount] = useState(0);
  ...
  it("should NOT inject duplicate useReactTracer inside map/filter chain with console.log", () => {
    const code = `
    ...
    // Should NOT have any __reactTracer2, __reactTracer3, etc.
    expect(out).not.toContain("__reactTracer2");
    expect(out).not.toContain("__reactTracer3");
  ...
  it("should handle deeply nested callbacks without duplicate useReactTracer", () => {
    const code = `
    ...
    // expect(countOccurrences(out, "__reactTracer.log(")).toBe(2);
    // expect(countOccurrences(out, "__reactTracer.error(")).toBe(1);
  });
});
