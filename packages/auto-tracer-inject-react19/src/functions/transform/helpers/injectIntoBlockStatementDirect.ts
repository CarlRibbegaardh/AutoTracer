 ...
 * a `useReactTracer` hook is available for state labeling.
 *
...
export function injectIntoBlockStatementDirect(
  ...
  let hasBareUseReactTracerCall = false;
    ...
    if (t.isVariableDeclaration(stmt)) {
      for (const decl of stmt.declarations) {
        if (decl.init && t.isCallExpression(decl.init)) {
          ...
          if (t.isIdentifier(callee, { name: "useReactTracer" })) {
      ...
      if (t.isIdentifier(callee, { name: "useReactTracer" })) {
        hasBareUseReactTracerCall = true;
      }
    }
  }
  ...
  if (hasBareUseReactTracerCall && !existingTracerIdentifier) {
    return;
  }
  ...
  const isReactHookCallee = (node: t.Node | null | undefined): node is t.Identifier => {
    return !!node && t.isIdentifier(node) && /^use[A-Z]/.test(node.name) && node.name !== "useReactTracer";
  };
  ...
  for (let i = 0; i < blockStatement.body.length; i++) {
    ...
    if (t.isVariableDeclaration(stmt) && stmt.declarations.length === 1) {
      ...
      if (hookName === "useReactTracer") continue;
  ...
  // ALWAYS inject useReactTracer for component tracking, even if no hooks to label
  // The useReactTracer() call provides:
  ...
  if (!tracerId) {
    tracerId = t.identifier("__reactTracer");
    const useReactTracerInit = t.callExpression(t.identifier("useReactTracer"), [
      t.objectExpression([
        t.objectProperty(t.identifier("name"), t.stringLiteral(componentName)),
      ]),
    ]);
    const tracerDecl = t.variableDeclaration("const", [
      t.variableDeclarator(tracerId, useReactTracerInit),
    ]);
  ...
  // If no hooks to label and no console calls, we're done (but useReactTracer was still injected above)
