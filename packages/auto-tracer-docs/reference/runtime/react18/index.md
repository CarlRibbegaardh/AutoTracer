...
If tracing starts correctly but the output format or styling is wrong for your workflow, read [`enabled`](./config/enabled), [`outputMode`](./config/outputMode), and [`colors`](./config/colors) together. If you use the Vite plugin, also see [ReactTracer Theme API](/themes/react18/api) and [ReactTracer Example Themes](/themes/react18/examples), where local theme files can override overlapping project defaults.
