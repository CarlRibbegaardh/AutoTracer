...
Instrumented components are never considered empty — they always appear in the output regardless of this setting. Components are normally instrumented by [`@autotracer/plugin-vite-react18`](/guide/installation-react-vite), which injects `useReactTracer` automatically.
