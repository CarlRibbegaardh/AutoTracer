...
After initialization, use the [Dashboard workflow](/dashboard/webapps) as the normal control surface in browser-based internal web apps when that workflow is mounted. The lower-level `globalThis.autoTracer.reactTracer.start()`, `stop()`, and `isEnabled()` controls remain useful in tests, automation, and other non-Dashboard setups. See [Runtime Control](/api/react18#runtime-control-global-api).
