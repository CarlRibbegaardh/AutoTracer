...
`endTriggerFunctionName` is a runtime configuration option for `reactTracer()` - the initializer of the `@autotracer/react19` React component render tracing library. It automatically stops tracing when a component display name matches the configured pattern.
...
After initialization, use `globalThis.autoTracer.reactTracer.setEndTrigger(...)`. See [Runtime Control](/api/react19#runtime-control-global-api).
