...
`includeReconciled` is a runtime configuration option for `reactTracer()` - the initializer of the `@autotracer/react19` React component render tracing library. It controls whether non-tracked components that React reconciled without re-rendering are shown in the tree.
