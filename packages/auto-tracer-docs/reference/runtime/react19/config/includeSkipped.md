...
`includeSkipped` is a runtime configuration option for `reactTracer()` - the initializer of the `@autotracer/react19` React component render tracing library. It controls whether non-tracked components that React processed internally without executing the component function are shown in the tree.
