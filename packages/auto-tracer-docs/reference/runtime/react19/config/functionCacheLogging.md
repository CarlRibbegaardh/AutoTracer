...
`functionCacheLogging` is a runtime configuration option for `reactTracer()` - the initializer of the `@autotracer/react19` React component render tracing library. It logs cache statistics after each render cycle.
