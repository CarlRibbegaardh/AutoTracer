...
- Component is tracked (has useReactTracer or matches auto-inject pattern)
...
- Counter resets only when the page reloads or the ReactTracer instance is recreated
...
The canonical `outputMode` controls the format of the console output.

- `outputMode: "copy-paste"` maps to `treeRenderingMode: "lineart"`
- `outputMode: "devtools"` maps to `treeRenderingMode: "group"`

#### Tree Rendering Mode: `lineart`

**Behavior:**

- Uses traditional text-based indentation with box-drawing characters (`├─`, `└─┐`)
- Best for copy-pasting logs into tickets or documents
- Preserves exact visual structure in text
...
#### Tree Rendering Mode: `group`

**Behavior:**

- Uses the browser's native `console.group()` API
- Best for interactive debugging in DevTools
- Allows expanding/collapsing component subtrees
- Components with children are rendered as groups
- Leaf components are rendered as logs
- Markers (collapsed empty levels) are skipped, allowing the next visible component to nest directly under the parent
...
**When enabled:** Set `internalLogLevel: "trace"` in options
...
With `internalLogLevel: "trace"`, markers show the absolute depth and node count:
...
**Result**: Only tracked components visible in DevTools-friendly grouped output.
