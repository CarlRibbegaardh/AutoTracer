...
This application is specifically designed to stress-test the `@autotracer/react18` library and identify performance bottlenecks in real-world scenarios. It simulates a complex Material-UI based single-page application with:
...
When ReactTracer hooks into every component, large state updates cause:
...
With ReactTracer tracking state changes:
...
- ReactTracer must track entire render cascade
...
```bash
pnpm --filter @autotracer/perf-test-mui dev
```

### Build

```bash
pnpm --filter @autotracer/perf-test-mui build
```
...
4. **Expected**: Measure initial render time with ReactTracer enabled
...
4. **Expected**: Identify if ReactTracer interferes with memo optimization

## Debugging Hooks

The app includes ReactTracer initialization in `main.tsx`:
...
2. **Hook Injection Overhead**
   - Every component gets `useReactTracer` injected
...
4. **Context Propagation**
   ...
   - ReactTracer must track entire cascade

## Next Steps

1. **Profile with React DevTools Profiler**
   - Identify which components cause the most re-renders
   - Measure impact of ReactTracer on render time
...
3. **Comparative Benchmarks**
   - Run app with ReactTracer disabled
...
- **ReactTracer** - Performance monitoring (under test)
...
```
src/
├── main.tsx              # Entry point with ReactTracer init
