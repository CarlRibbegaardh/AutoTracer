# Microfrontend ReactTracer Demo Suite

This is a **working microfrontend application** demonstrating ReactTracer integration in Module Federation architectures. The suite consists of one host application that dynamically loads two remote microfrontends using the official `@module-federation/vite` plugin.
...
graph TB
    subgraph "Host Application :5190"
        H[Host App]
        H -->|Initializes Once| AT[Shared ReactTracer Instance]
    end

    subgraph "Remote 1 :5191"
        R1[Remote 1 App]
        R1 -->|Exposes| C1[Counter & TodoList]
        R1 -->|Plugin Injects| I1[useReactTracer calls]
    end

    subgraph "Remote 2 :5192"
        R2[Remote 2 App]
        R2 -->|Exposes| C2[Timer & Form]
        R2 -->|Plugin Injects| I2[useReactTracer calls]
    end
...
- **Features**:
  ...
  - ReactTracer integration for host components
...
**Note**: When running remotes standalone, they initialize their own ReactTracer instance. When loaded via the host, they share the host's singleton instance.
...
### 1. Singleton Shared ReactTracer Instance

**Current Setup**: Single ReactTracer instance shared across all apps

- Host initializes ReactTracer **once** before rendering
- React, ReactDOM, and `@autotracer/react18` are configured as **singletons** in Module Federation
- All three apps share the same ReactTracer instance
- Remote components automatically use the host's ReactTracer

**What This Demonstrates**:

✅ **Unified Observability**: ReactTracer sees all component renders across the entire federated application as one cohesive React tree<br>
...
- **Host**: Manual instrumentation with explicit `useReactTracer()` and `logger.labelState()` calls
- **Remote 1 & 2**: Automatic injection via `@autotracer/plugin-vite-react18` Vite plugin

**What This Demonstrates**:

✅ **Plugin Transforms First**: ReactTracer plugin runs before Module Federation plugin in the build pipeline<br>
✅ **Zero Code Changes**: Remote components need no ReactTracer imports or hooks - fully automatic<br>
...
- **Shared Singletons**: React, ReactDOM, and ReactTracer must all be `singleton: true`
...
federation({
  ...
  shared: {
    ...
    "@autotracer/react18": { singleton: true },
  },
});
```
...
### Why ReactTracer IS Shared (and Must Be)

The `@autotracer/react18` package **must** be shared as a singleton because:

1. **Single React DevTools Hook**: React's DevTools hook is global, and ReactTracer attaches to it
2. **Unified Trace Output**: Multiple ReactTracer instances would create separate, conflicting trace streams
...
│   │   ├── main.tsx        # ReactTracer initialization
│   │   └── ...
│   ├── package.json
...
│   │   ├── main.tsx        # Conditional ReactTracer init
│   │   └── ...
│   ├── package.json
...
1. **Check Console**: ReactTracer logs are enabled with `internalLogLevel: "trace"`
...
**ReactTracer Not Showing Traces**

- Check browser console for "ReactTracer: Global render monitor initialized"
- Verify `enabled: true` in reactTracer config
- For manual instrumentation, ensure `useReactTracer()` is called
...
1. **Single Unified Trace Stream**: One ReactTracer instance logs all components from all apps
...
6. **No Conflicts**: Single shared React + ReactTracer instance = clean, unified output

## What This Demo Proves

✅ **ReactTracer works seamlessly in Module Federation architectures**<br>
✅ **Singleton sharing is critical** - React, ReactDOM, and ReactTracer must all be singletons<br>
...
1. ✅ ~~Attempt to share ReactTracer instance via Module Federation~~ **COMPLETED**
