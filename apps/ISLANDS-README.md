# React Islands Architecture Demo with ReactTracer

This demo showcases **truly independent React islands** - multiple React applications that bundle their own React and ReactTracer, requiring zero coordination from the host page beyond a simple DevTools hook (which the React DevTools browser extension provides automatically).

## What Makes This Special

**Each island is completely self-contained:**

- ✅ Bundles its own React + ReactDOM + ReactTracer
...
**Real-world use case:** A developer building a React widget for a CMS can develop with full ReactTracer observability without requiring any setup from the CMS administrators. Just drop in the UMD bundle and it works.
...
graph TB
    ...
    subgraph "HTML Page :5200"
        ...
        H -->|Loads & Mounts| I1[Island 1 UMD<br/>React + ReactTracer bundled]
        H -->|Loads & Mounts| I2[Island 2 UMD<br/>React + ReactTracer bundled]
    end

    subgraph "Island 1 :5201"
        R1[React App 1<br/>Counter & Todo]
        R1 -->|Builds to| U1[island.umd.js<br/>~150KB with React]
        R1 -->|Plugin Injects| P1[useReactTracer calls]
    end

    subgraph "Island 2 :5202"
        R2[React App 2<br/>Timer & Form]
        R2 -->|Builds to| U2[island.umd.js<br/>~150KB with React]
        R2 -->|Plugin Injects| P2[useReactTracer calls]
    end

    RDT -.->|Provides| HOOK
    I1 -->|Initializes Own| AT1[ReactTracer Instance 1]
    I2 -->|Initializes Own| AT2[ReactTracer Instance 2]
...
3. **Shared across React instances** - Multiple React instances (one per island) all use the same hook to report their renders to ReactTracer.
...
- **Responsibilities**:
  - Initializes ReactTracer once for the entire page
...
- **Features**:
  ...
  - Automatic ReactTracer instrumentation via Vite plugin
...
- **Tracing**: @autotracer/react18 (workspace package)
- **Instrumentation**: @autotracer/plugin-vite-react18 (automatic injection)
- **Dependencies**: Each island bundles React 18.3.1 + ReactDOM + ReactTracer (~150KB per island)
...
Turbo automatically builds the UMD bundles first, then starts all three dev servers. The island dev servers serve both source files (for standalone mode) and the built UMD bundles (for the loader).
...
- ReactTracer logs for all component renders
- Separate React renderer IDs for each island
- Shared render cycle counter (both islands use the same ReactTracer global state)

### Development Workflow

**Testing the integrated demo:** Run `pnpm dev:islands` and open http://localhost:5200

**Standalone island development:** To develop a single island with HMR:
...
**How dev mode serves UMD bundles:** Each island's Vite dev server includes a custom middleware plugin that serves the built `island.umd.js` file from the `dist/` folder when requested by the loader, while simultaneously serving source files with HMR for standalone development
...
build: {
  ...
  rollupOptions: {
    ...
    // React, ReactDOM, and ReactTracer are bundled into the UMD
  }
}
```
...
- Result: ~150KB UMD bundle including React + ReactDOM + ReactTracer

### ReactTracer Plugin During Build

The Vite plugin runs during build and:

- Injects `useReactTracer()` calls into every component
...
```typescript
// src/mount.tsx
export async function mount(elementId: string) {
...
**Important:** Each island calls `reactTracer()`, but only the first call actually initializes (ReactTracer is a singleton). Subsequent calls are safely ignored.
...
<!-- 2. Load Island 1 UMD (contains React + ReactTracer) -->
<script src="http://localhost:5201/island.umd.js"></script>
<script>
  window.Island1.mount("island-1"); // Initializes ReactTracer
</script>

<!-- 3. Load Island 2 UMD (contains React + ReactTracer) -->
<script src="http://localhost:5202/island.umd.js"></script>
<script>
  window.Island2.mount("island-2"); // ReactTracer already initialized, no-op
</script>
```
...
- No shared ReactTracer UMD - each island bundles it
...
- ReactTracer is a singleton even though each island bundles it

### Multiple React Instances, One ReactTracer

**What's happening under the hood:**

1. Island 1 loads → Its bundled React creates renderer #1
2. Island 1's ReactTracer initializes and installs the render hook
3. Island 2 loads → Its bundled React creates renderer #2
4. Island 2's ReactTracer init call is ignored (already initialized)
5. Both React instances use the same DevTools hook
6. ReactTracer sees renders from both via the shared hook
...
✅ **Each Island Bundles Everything**: React + ReactDOM + ReactTracer all included<br>
...
✅ **Automatic Instrumentation**: Plugin injects ReactTracer without code changes<br>
...
1. **Develop your island standalone:**
   ...
   pnpm dev
   # Opens http://localhost:5201 with full ReactTracer observability
   ```

2. **Make changes, see traces immediately:**
   - Add components, hooks, state
   - ReactTracer plugin automatically instruments everything
...
4. **Deploy to CMS:**
   ...
   - Add to page template:
     ...
     <script>
       window.Island1.mount("my-widget");
     </script>
     ```

5. **Debug in production:**
   - Install React DevTools extension
   - Full ReactTracer observability without any CMS configuration!
...
- ✅ **Required for ReactTracer to work without manual hook setup**

Without the extension, you need the fallback hook (shown in the demo loader).

### Bundle Size

Each island is ~150KB (React + ReactDOM + ReactTracer):
...
### ReactTracer in Production

Consider your strategy:

- **Dev/Test/QA only**: Use environment checks to disable in production
- **QA/Staging observability**: Keep ReactTracer enabled in test environments for debugging
...
- [@autotracer/react18 Package](../../packages/auto-tracer-react18/README.md)
- [@autotracer/plugin-vite-react18 Package](../../packages/auto-tracer-plugin-vite-react18/README.md)
...
```
🏝️ ReactTracer initialized for islands architecture
🟦 Island 1 mounted
🟩 Island 2 mounted
...
✅ **Islands architecture works with ReactTracer**<br>
✅ **UMD format enables script-tag loading**<br>
✅ **Multiple independent React apps can share ReactTracer**<br>
