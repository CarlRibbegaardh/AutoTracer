...
This app demonstrates the auto-injection of `useReactTracer` into React components.

### Setup Requirements:

1. **ReactTracer Initialization** (`src/main.tsx`) - Lazy-load `@autotracer/react18` before rendering any components:
...
**Mode: `opt-out`** - All components get `useReactTracer()` injected automatically, except:
...
- `main.tsx` - Entry point file, contains the lazy ReactTracer setup call instead
...
3. `useReactTracer()` calls are automatically injected as the first statement
...
4. In Sources tab, you can see the transformed code with injected `useReactTracer()` calls
...
reactTracer.vite({
  ...
  importSource: "@autotracer/react18", // Import useReactTracer from this package
