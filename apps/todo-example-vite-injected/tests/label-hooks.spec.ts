...
test.describe("Hook Labeling E2E Tests", () => {
  test("should label useState, useReducer, and custom hooks based on configuration", async ({
    ...
    // The primitive values inside the custom hooks (useState) are now inferred from the parent object!
    // Nested hook inference matches hook values against labeled object properties
    expect(
      pageLogs.some((log) =>
        log.includes("Initial state customHookResult.value: pattern-custom")
      )
    ).toBe(true);
    expect(
      pageLogs.some((log) =>
        log.includes("Initial state nestedHookResult.value: nested-custom")
      )
    ).toBe(true);

    // Updates
    // When the internal state of custom hooks changes, we see TWO state change logs:
    // 1. The primitive value inside the hook (internal useState) - now properly inferred!
    // 2. The hook's return value (the object) - correctly labeled with variable name

    // Custom hook internal state is now inferred as customHookResult.value
    expect(
      pageLogs.some(
        (log) =>
          (log.includes("State change customHookResult.value:") ||
    ...
    // Nested hook internal state is now inferred as nestedHookResult.value
    expect(
      pageLogs.some(
        (log) =>
          log.includes("State change nestedHookResult.value:") &&
