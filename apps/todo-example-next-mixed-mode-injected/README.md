...
- In Next.js, auto-injection is currently NOT enabled; client components import/call `useReactTracer` manually.
...
- Manual tracing only. Import from `auto-tracer` and call `useReactTracer()` inside client components as needed.
...
flowchart TD
	...
	B[Client Module with "use client"] --> C[Manual useReactTracer()]
	C --> E[Run App]
```
