...
This is the **host application** in a microfrontend architecture demonstration. It dynamically loads two remote microfrontends (`remote1` and `remote2`) using Vite Module Federation and demonstrates ReactTracer behavior across microfrontend boundaries.
...
- **ReactTracer Integration**: Demonstrates tracing across microfrontend boundaries
...
│   ├── main.tsx         # Entry point with ReactTracer initialization
...
**Note**: ReactTracer is **not** currently shared via Module Federation. Each application bundles its own ReactTracer instance. This allows investigation of how multiple ReactTracer instances behave in a microfrontend architecture.
...
The host lazy-loads ReactTracer **before** React renders in tracing-enabled builds:

```typescript
async function initializeAutoTracer(): Promise<void> {
...
Each remote application also initializes its own ReactTracer instance (for standalone mode), creating multiple independent tracing streams in the microfrontend setup.
...
```typescript
function App() {
  useReactTracer(); // Trace host component renders
  // ... component logic
}
```
...
This app is part of a three-app setup to investigate ReactTracer behavior in microfrontend scenarios:

1. **Independent ReactTracer Instances**: Each app has its own ReactTracer instance
