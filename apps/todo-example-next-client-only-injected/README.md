...
- reactTracer is initialized on the client via a minimal bootstrap component in `src/app/ReactTracerBootstrap.tsx`, rendered at the top of `layout.tsx`
...
Status: working demo (reactTracer initialized on client, Babel auto-injection enabled)
...
- Global tracing: `reactTracer()` is started by `ReactTracerBootstrap` as early as possible on the client
- Labeled logs: The Babel plugin auto-injects `useReactTracer` into client components for readable labels in logs
...
```mermaid
graph TD
	A[Next App Router layout "use client"] --> B[ReactTracerBootstrap]
	B -->|reactTracer| C[Attach to DevTools hook]
	C --> D[Render client components]
	D --> E[Labeled logs via injected useReactTracer]
```
