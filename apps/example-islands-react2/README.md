...
- **Automatic Tracing**: ReactTracer instrumentation injected at build time

## State Labels (Auto-Traced)

All state is automatically labeled by `@autotracer/plugin-vite-react18`:
...
This mode:
- Initializes its own ReactTracer instance
...
- `dist/auto-tracer-react18.umd.js` - ReactTracer UMD (for standalone)
...
- **Format**: UMD
- **Global Name**: `Island2`
- **External Dependencies**: React, ReactDOM, ReactTracer
- **Entry**: `src/mount.tsx`
- **Component Prefix**: `Island2` — all traced component names are prefixed as `Island2:ComponentName` so they are identifiable in the DevTools output when multiple islands are on the same page
...
- @autotracer/plugin-vite-react18 (automatic instrumentation)
